...
public static class MapSettingValueStringPatch
{
    ...
    {
        if ((int)value !=
            (int)MapType.LevelImposter - 1) // <-- MapSelectionGameSetting.TryGetInt subtracts 1 for some reason
            return true;
...
public static class MapTooltipPatch
{
    public static void Postfix(CreateGameOptions __instance)
    {
        ...
        while (__instance.mapTooltips.Length <= (int)MapType.LevelImposter)
            __instance.mapTooltips = __instance.mapTooltips.Add(LIConstants.MAP_STRING_NAME);

        // Add wordart banner
        var wordart = LIMapIconBuilder.Get().NameImage;
        while (__instance.mapBanners.Length <= (int)MapType.LevelImposter)
            __instance.mapBanners = __instance.mapBanners.Add(wordart); // IMAGE-SkeldBanner-Wordart (342x72)

        // Add crewmate background
        var background = PackagedResources.LoadSprite("LOBBY-Background.png");
        while (__instance.bgCrewmates.Length <= (int)MapType.LevelImposter)
            __instance.bgCrewmates = __instance.bgCrewmates.Add(background); // BACKGROUND-4 players (515x895)
    }
}
...
public static class MapConfirmCreatePatch
{
    public static void Prefix(ConfirmCreatePopUp __instance)
    {
        ...
        while (__instance.mapLogos.Length <= (int)MapType.LevelImposter)
            __instance.mapLogos = __instance.mapLogos.Add(mapIcon.NameImage);

        while (__instance.mapBanners.Length <= (int)MapType.LevelImposter)
            __instance.mapBanners = __instance.mapBanners.Add(mapIcon.MapImage);
    }
}